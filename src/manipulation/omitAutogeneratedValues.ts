...
export const omitAutogeneratedValues = <T extends DomainObject<Record<string, any>>>(obj: T): T => {
  ...
  const objWithoutBaseCaseAutogeneratedValues = new Constructor(omit(obj, BASE_CASE_AUTOGENERATED_KEYS));

  // if this is a value object, then base case is sufficient
  if (isValueObject) return objWithoutBaseCaseAutogeneratedValues;
  ...
  if (isEntity) {
    ...
    return objWithoutBaseCaseAutogeneratedValues; // otherwise, if uuid is not part of unique keys, then the base case applies here too
  }
