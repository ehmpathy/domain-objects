...
export const omitAutogeneratedValues = <T extends DomainObject<Record<string, any>>>(obj: T): T => {
  ...
  if (isEntity) {
    ...
    if (uniqueKeys.flat().includes('uuid')) return new Constructor({ ...objWithoutBaseCaseAutogeneratedValues, uuid: (obj as any).uuid }); // if unique keys include `uuid`, then `uuid` is not autogenerated - add it back
